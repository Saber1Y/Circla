    ...
    <main className="mx-auto max-w-[1240px] px-7 py-6 max-md:px-4">
      <header className="flex items-center justify-between gap-4">
        <a href="/" className="font-[var(--font-newsreader)] text-[17px] font-medium tracking-tight text-[#101114]">
          CIRCLA<span className="ml-1 font-[var(--font-sora)] text-[11px] font-bold tracking-widest text-[#0052FF]">/BASE</span>
        </a>
      ...
      <section className="mx-auto max-w-[800px] py-12 text-center md:py-16">
        <div className="hide-in-tma">
          <div className="animate-slide-in inline-flex items-center rounded-full border border-blue-200 bg-blue-50 px-4 py-1.5 text-xs font-semibold uppercase tracking-wide text-[#0052FF] delay-100">Built on Base • Powered by B20 Stocks</div>
          ...
          <div className="animate-slide-in mt-8 flex flex-wrap justify-center gap-3 delay-400">
            <a href={TG_APP_URL} target="_blank" rel="noreferrer" className="rounded-full bg-[#0052FF] px-7 py-3.5 text-sm font-bold text-white shadow-sm hover:opacity-90">Launch in Telegram</a>
          ...
          {isLive && (
            <p className="mt-4 text-xs text-[#918d85]">Live Sepolia vault <a href={vaultLink} target="_blank" className="font-mono text-[#0052FF] hover:underline">{VAULT.slice(0, 6)}…{VAULT.slice(-4)}</a> · <a href={vaultLink} target="_blank" className="underline">Basescan ↗</a></p>
          )}
        </div>
        {/* TMA direct route */}
      ...
      <section className="mt-6 overflow-hidden rounded-[24px] border border-[#e3dfd7] bg-white">
        ...
        <div className="grid grid-cols-1 gap-4 bg-[#fcfaf8] p-4 md:grid-cols-[1.15fr_380px] md:p-6">
            ...
            <div className="mt-3 space-y-2.5">
              ...
              <div className="rounded-2xl border border-[#e3dfd7] bg-[#f5f3ee] p-3">
                ...
                <p className="mt-1 flex items-center gap-1.5 font-mono text-[10px] text-[#77736c]">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#0052FF]" /> sepolia proof · <a href={vaultLink} target="_blank" className="underline hover:text-[#101114]">Basescan</a> · scaledBalanceOf
                </p>
              </div>
          ...
          <div className="rounded-2xl border border-[#e3dfd7] bg-white p-5 shadow-[0_2px_10px_rgba(16,17,20,0.04)]">
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-[#EFF6FF] px-2 py-1 text-[10px] font-bold text-[#0052FF]">Live</span>
            </div>
            <div className="mt-4 flex items-center gap-4">
              <div className="relative h-[88px] w-[88px]">
                <div className="absolute inset-0 rounded-full border-[10px] border-[#e3dfd7]" />
                <div className="absolute inset-0 rounded-full border-[10px] border-[#0052FF] border-r-transparent border-b-transparent" style={{ transform: "rotate(-45deg)" }} />
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-[11px] font-bold text-[#101114]">B20</span>
                </div>
              </div>
              <div className="flex-1 space-y-2 text-xs">
                <div className="flex items-center justify-between rounded-full bg-[#EFF6FF] px-3 py-1.5"><span className="flex items-center gap-1.5 font-bold text-[#101114]"><span className="h-2 w-2 rounded-full bg-[#0052FF]" />62% NVDAc</span><span className="font-mono text-[11px] text-[#77736c]">800k</span></div>
        ...
        <div className="flex flex-wrap gap-2 border-t border-[#f0ede8] bg-[#fcfaf8] px-4 py-3 md:px-6">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#e3dfd7] bg-white px-3.5 py-2 text-xs font-medium text-[#5e5b57]"><span className="h-2 w-2 rounded-full bg-[#0052FF]" />B20 Multipliers</span>
