    ...
    <main className="mx-auto max-w-[1180px] px-7 py-6 max-md:px-4">
      <header className="flex items-center justify-between gap-4">
        ...
        <div className="ml-auto hidden items-center gap-2 text-xs text-[#77736c] md:flex">
          ...
          <a href={vaultLink} target="_blank" rel="noreferrer" className="font-semibold hover:text-[#101114]">Vault ↗</a>
          <a href="/" className="font-semibold hover:text-[#101114]">Landing →</a>
        </div>
      ...
      <section className="max-w-[880px] pb-6 pt-8">
        ...
        <div className="mt-4 flex items-center gap-2 text-xs text-[#77736c]">
          <span className={`h-[7px] w-[7px] rounded-full ${account ? "bg-[#42a85f] shadow-[0_0_0_4px_rgba(66,168,95,0.15)]" : "bg-[#bcb8b0]"}`} /> {account ? `${account.slice(0, 6)}…${account.slice(-4)} connected` : "Wallet not connected"} <span className="text-[#c4c0b8]">/</span> {chain.name}
        </div>
      </section>
      ...
      <section className="grid grid-cols-2 gap-3 py-4 max-md:grid-cols-1">
