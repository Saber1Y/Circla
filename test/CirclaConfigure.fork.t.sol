...
contract CirclaConfigureForkTest is Test {
    ...
    function testConfigureAllAsOwner() external {
        ...
        vm.prank(OWNER);
        REGISTRY.configureAsset(SNDKC, SNDKC_FEED, 8, 10, 100_000_000, true);
        vm.prank(OWNER);
        REGISTRY.configureAsset(SPCXC, SPCXC_FEED, 8, 10, 100_000_000, true);
        vm.prank(OWNER);
        REGISTRY.configureAsset(TSLAC, TSLAC_FEED, 8, 10, 100_000_000, true);
        vm.prank(OWNER);
        REGISTRY.configureAsset(COINC, COINC_FEED, 8, 10, 100_000_000, true);
        vm.prank(OWNER);
        REGISTRY.configureAsset(CRCLC, CRCLC_FEED, 8, 10, 100_000_000, true);
        vm.prank(OWNER);
        REGISTRY.configureAsset(INTCC, INTCC_FEED, 8, 10, 100_000_000, true);
        ...
        assertTrue(REGISTRY.getAsset(SNDKC).enabled);
        assertTrue(REGISTRY.getAsset(SPCXC).enabled);
        assertTrue(REGISTRY.getAsset(TSLAC).enabled);
        assertTrue(REGISTRY.getAsset(COINC).enabled);
        assertTrue(REGISTRY.getAsset(CRCLC).enabled);
        assertTrue(REGISTRY.getAsset(INTCC).enabled);
    }
}
